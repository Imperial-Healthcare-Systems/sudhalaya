-- Suddhalaya backend — Phase 4.1: FIFO fulfilment in place_order.
-- 1) Ensure a default warehouse exists and backfill existing variant stock into an
--    initial batch, so orders keep working the moment batches take over. Idempotent.
-- 2) create-or-replace place_order to deduct via deduct_fifo (oldest mfg first),
--    keeping the exact same pricing/GST/shipping/coupon logic + JSON return contract.

-- 1) default warehouse + backfill --------------------------------------------
insert into public.warehouses (name, code, city, state, is_default, active)
  select 'Main Warehouse', 'MAIN', 'Bengaluru', 'Karnataka', true, true
  where not exists (select 1 from public.warehouses);

insert into public.inventory_batches
  (variant_id, warehouse_id, batch_no, mfg_date, qty_received, qty_remaining)
  select pv.id,
         (select id from public.warehouses where is_default order by id limit 1),
         'INIT', current_date, pv.stock, pv.stock
    from public.product_variants pv
   where pv.stock > 0
     and not exists (select 1 from public.inventory_batches b where b.variant_id = pv.id);

-- 2) place_order with FIFO ----------------------------------------------------
...
begin
  ...
  -- validate + lock variant rows, build authoritative line items (now carrying vid)
  ...
  -- FIFO stock deduction per line (oldest manufacture date first, across batches).
  -- deduct_fifo RAISEs 'Insufficient stock' if a variant's batches can't cover it.
