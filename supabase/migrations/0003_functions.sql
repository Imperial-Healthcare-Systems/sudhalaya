-- Suddhalaya backend — SECURITY DEFINER functions (Phase 1)
-- Order placement and coupon validation run server-side so prices, stock, GST,
-- shipping and coupons are authoritative and never trusted from the client.

-- Validate a coupon against a cart subtotal. Returns the computed discount + meta.
...
begin
  ...
  if p_subtotal < c.min_cart then
    return jsonb_build_object('valid', false, 'reason', 'Add ₹'||(c.min_cart)::int||'+ to use this code.');
  end if;
  ...
  if c.type = 'pct' then disc := round(p_subtotal * c.value/100, 2);
  else disc := least(c.value, p_subtotal); end if;
...
-- Place an order atomically: validate items against live price/stock, compute
-- GST-inclusive totals + shipping + coupon, insert order/items/timeline, deduct
-- stock, upsert the customer, and return the created order summary.
create or replace function public.place_order(
  p_items          jsonb,          -- [{"sku":"SDL-GHEE-500","qty":1}, ...] (variant SKUs)
  p_customer       jsonb,          -- {"name","email","phone"}
  p_ship           jsonb,          -- {"name","line","city","state","pin"}
  p_payment_method text,           -- 'upi' | 'card' | 'cod'
...
begin
  ...
  -- validate + lock variant rows, build authoritative line items
  for v_item in select * from jsonb_array_elements(p_items) loop
    v_qty := greatest(1, coalesce((v_item->>'qty')::int, 1));
    select pv.sku, pv.label, pv.price, pv.stock, p.id as pid, p.gst as pgst, p.name as pname, p.ship_fee as pship
      into v_variant
    ...
    v_lines := v_lines || jsonb_build_object(
      'sku', v_variant.sku, 'name', v_variant.pname, 'variant', v_variant.label,
  ...
  if p_coupon is not null and length(p_coupon) > 0 then
    ...
    if found and v_gross >= v_coupon.min_cart
       and (v_coupon.expires is null or v_coupon.expires >= current_date) then
      if v_coupon.type = 'pct' then v_disc := round(v_gross * v_coupon.value/100, 2);
      else v_disc := least(v_coupon.value, v_gross); end if;
  ...
  -- GST is inclusive in the stored price; split the discounted goods into base + tax
  select coalesce(sum(
  ...
  -- shipping: free over threshold, else flat + per-product surcharges
  ...
  if p_payment_method = 'cod'
     and (not v_cod_enabled or (v_cod_max > 0 and v_total > v_cod_max)) then
  ...
  insert into public.orders(
      order_no, user_id, customer_name, email, phone,
      ship_name, ship_line, ship_city, ship_state, ship_pin,
  ...
  update public.product_variants pv set stock = stock - li.qty
    from jsonb_to_recordset(v_lines) as li(sku text, qty int)
    where pv.sku = li.sku;
  ...
  if v_cust_id is not null then
    update public.orders set customer_id = v_cust_id where id = v_order_id;
  end if;
...
-- Let the storefront call these via PostgREST RPC (definer enforces the rules).
